...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    private static function getEntityDataClassById(array $arHlblock): string
    {
         ...
         */
        // var_dump($arHlblock);
        $hlblock = \Bitrix\Highloadblock\HighloadBlockTable::getById($arHlblock["ID"])->fetch(); //где ID - id highloadblock блока в который будем добавлять элементы
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
             ...
             */
            $result = CUserFieldEnum::GetList(array(), array(
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        while ($arAgent = $rsAgents->fetch()) {
            ...
            */
            foreach($arTypeAgents as $ag){
                if($ag["ID"] === $arAgent["UF_TYPE_ACTIVITY"]){
                    $arAgent["UF_TYPE_ACTIVITY"] = $ag["VALUE"];
                }
            }
            $file_id = $arAgent["UF_PHOTO"];
            $arAgent["UF_PHOTO"] = CFile::GetPath($file_id);           
